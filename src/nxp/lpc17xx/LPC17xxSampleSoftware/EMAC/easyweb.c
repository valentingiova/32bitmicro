...
// function replaced to handle LPC17xx A/D converter.
  ...
  ADC -> ADCR = 0x01203104;       // Setup A/D: 12-bit AIN2 @ 
  do {
    val = ADC -> ADGDR;                        // Read A/D Data Register
  } while ((val & 0x80000000) == 0);     // Wait for end of A/D Conversion
  ADC -> ADCR &= ~0x01000001;                  // Stop A/D Conversion
...
// function replaced to handle LPC17xx A/D converter.
  ...
  ADC -> ADCR &= ~0x01000002;                  // Stop A/D Conversion
  //val = (val >> 4) & 0x0FFF;             // Extract AIN2 Value
